...
class PDFViewer(Frame):
    ...
    def _init_ui(self):
        ...
        self.canvas = DisplayCanvas(canvas_frame, page_height=h, page_width=w)
        self.canvas.pack()

        self.grid(row=0, column=0, sticky='news')

        self.master.minsize(height=h, width=w)
        self.master.maxsize(height=768, width=1024)
    ...
    def _decrypt_pdf(self):
    	path = self.paths[self.pathidx]
    	filename = os.path.basename(path)
    	if filename.split('.')[-1].lower() in ['sec']:
    		text = simpledialog.askstring('Enter Password', 'Enter your password here:',show='*')
    		if text == '' or text is None:
    			return
    		path = self.paths[self.pathidx]
    		filename = os.path.basename(path)
    		print(path)
    		print(text)
    	if filename.split('.')[-1].lower() in ['pdf']:
    		print(self.paths)
    		self._load_file()
    ...
    def _open_file(self):
    	self._clear()
    	paths = filedialog.askopenfilenames(filetypes=[('sec files', '*.sec'), ('all files', '.*')],initialdir=os.getcwd(),title="Select files", multiple=False)
    	if not paths or paths == '':
    		return
    	paths = [path for path in paths if os.path.basename(path).split('.')[-1].lower() in ['sec','pdf']]
    ...
    def _open_dir(self):
        dir_name = filedialog.askdirectory(initialdir=os.getcwd(), title="Select Directory")
